...
export async function POST(req: Request) {
  ...
  if (Number.isNaN(startsAt.getTime())) {
    return NextResponse.json({ ok: false, error: "invalid startsAtIso" }, { status: 400 });
  }

  if (!String(body.clientName || "").trim()) {
    return NextResponse.json({ ok: false, error: "clientName required" }, { status: 400 });
  }
  ...
  // Pick the assignee with fewest appointments that day.
  const dayStart = new Date(startsAt);
  dayStart.setUTCHours(0, 0, 0, 0);
  const dayEnd = new Date(dayStart.getTime() + 24 * 60 * 60_000);
  ...
  const summary = `Speedwell Law Discovery Call — ${String(body.clientName).trim()}`;
  ...
  return NextResponse.json({ ok: true, appointmentId: appt.id, assignedTo: chosen, eventId: ev.id });
}
