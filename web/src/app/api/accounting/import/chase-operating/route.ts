...
export async function POST(request: Request) {
  ...
  // Link to (or create) a firm financial account.
  const accountName = "OPERATING";
  const account = await prisma.financialAccount.upsert({
    where: { firmId_name: { firmId: user.activeFirmId, name: accountName } },
    update: { kind: "OPERATING_BANK", active: true },
    create: { firmId: user.activeFirmId, name: accountName, kind: "OPERATING_BANK", active: true },
  });
