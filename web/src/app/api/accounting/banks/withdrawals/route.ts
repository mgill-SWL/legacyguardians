...
export async function POST(req: Request) {
  ...
  const toBePrinted = body?.toBePrinted !== undefined ? !!body.toBePrinted : true;
  ...
  if (method !== "CHECK") return NextResponse.json({ ok: false, error: "Only method=CHECK is supported in v1" }, { status: 400 });
  if (!toBePrinted && !checkNumber) return NextResponse.json({ ok: false, error: "checkNumber required" }, { status: 400 });
  if (!toBePrinted && !isDigits(checkNumber)) return NextResponse.json({ ok: false, error: "checkNumber must be digits when not printing" }, { status: 400 });
  ...
  let checkId: string;
  try {
    const created = await prisma.operatingCheck.create({
      data: {
  ...
  if (!toBePrinted && isDigits(checkNumber)) {
    ...
    if (!existingSeq) {
      ...
      await prisma.financialAccountCheckSequence.update({ where: { id: existingSeq.id }, data: { nextNumber: proposedNext } });
    }
  }

  return NextResponse.json({ ok: true, checkId, printUrl: toBePrinted ? `/management/accounting/checks/${checkId}/print` : null });
}
